...
void main() {
  ...
  test('same train within 1h -> one group; >1h -> separate groups', () async {
    await DatabaseService.instance.insertRecord(
      mkRecord(uniqueId: 'a', receivedMs: _base, train: 'T1', direction: 1),
    );
    await DatabaseService.instance.insertRecord(
      mkRecord(
        uniqueId: 'b',
    ...
    final merged = items.whereType<MergedTrainRecord>().toList();
  ...
  test(
    'union: a record bridging train-key and loco-key groups merges them',
    () async {
  ...
  test(
    'isUngroupable singleton is hidden when hideUngroupable is on',
    () async {
      await DatabaseService.instance.insertRecord(
        mkRecord(
          uniqueId: 'u',
  ...
  test(
    'groupId stays stable when an older (out-of-order) record folds in',
    () async {
      // Insert two same-train records -> group forms with groupId = 'g1'
      // (the lexicographically smaller uniqueId, assigned at creation).
      await DatabaseService.instance.insertRecord(
        mkRecord(uniqueId: 'g1', receivedMs: _base, train: 'TG', direction: 1),
      );
      await DatabaseService.instance.insertRecord(
        mkRecord(
          uniqueId: 'g2',
      ...
      final before =
          (await RecordsFeed.itemContaining(
                mkRecord(uniqueId: 'g1', receivedMs: _base, train: 'TG'),
              ))
  ...
  test(
    'merged summary surfaces complete data when the newest member is broken',
    () async {
  ...
  test(
    'merged card keeps the lbjClass prefix when only an older member has it',
    () async {
      ...
      final item = await RecordsFeed.itemContaining(
        mkRecord(
          uniqueId: '1782098432000_8040',
  ...
  test('garbled train/loco values are not used as grouping keys', () async {
    ...
    final hidden = (await RecordsFeed.fetchPage(
      limit: 100,
      cursor: null,
    )).items;
  ...
  test(
    'rebuildMergeCache regenerates stale derived columns and summaries',
    () async {
      await DatabaseService.instance.updateSettings({
        'mergeRecordsEnabled': 1,
        'hideUngroupableRecords': 1,
      ...
      expect(
        page.whereType<TrainRecord>().where((r) => r.uniqueId == 'stale1'),
        isEmpty,
      );
